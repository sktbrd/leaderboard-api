 ...
 SELECT 
  ...
  (
    SELECT json_agg(json_build_object(
      'body', child.body,
      'author', child.author,
      'permlink', child.permlink,
      'created', child.created
    ) ORDER BY child.created ASC)
    FROM (
      SELECT body, author, permlink, created
      FROM comments child
      WHERE child.parent_author = c.author
        AND child.parent_permlink = c.permlink
